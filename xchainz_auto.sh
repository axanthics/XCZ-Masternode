#!/bin/bash
# XChainZ Masternode Setup Script V1 for Ubuntu 16.04 LTS
#
...
function stop_daemon {
    if pgrep -x 'xchainzd' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop xchainzd${NC}"
        xchainz-cli stop
        sleep 30
        if pgrep -x 'xchainzd' > /dev/null; then
            echo -e "${RED}xchainzd daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            sudo pkill -9 xchainzd
            sleep 30
            if pgrep -x 'xchainzd' > /dev/null; then
                echo -e "${RED}Can't stop xchainzd! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
cd ~/.xchainz/ && wget https://github.com/axanthics/xcz/releases/download/v1.0.0/bootstrap.zip
cd ~/.xchainz/ && unzip bootstrap.zip	
# Create xchainz.conf
cat <<EOF > ~/.xchainz/xchainz.conf
rpcuser=$rpcuser
rpcpassword=$rpcpassword
...
======================================================================== \a"
echo -e "${GREEN}xchainz_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
echo -e "========================================================================
Use your mouse to copy the whole string above into the clipboard by
tripple-click + single-click (Dont use Ctrl-C) and then paste it 
into your ${GREEN}masternode.conf${NC} file and replace:
    ${GREEN}xchainz_mn1${NC} - with your desired masternode name (alias)
    ${GREEN}TxId${NC} - with Transaction Id from masternode outputs
    ${GREEN}TxIdx${NC} - with Transaction Index (0 or 1)
     Remember to save the masternode.conf and restart the wallet!
To introduce your new masternode to the xchainz network, you need to
...
========================================================================
To view masternode configuration produced by this script in xchainz.conf:
${GREEN}cat ~/.xchainz/xchainz.conf${NC}
Here is your xchainz.conf generated by this script:
-------------------------------------------------${GREEN}"
echo -e "${GREEN}xchainz_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
cat ~/.xchainz/xchainz.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit xchainz.conf, first stop the xchainzd daemon,
then edit the xchainz.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the xchainzd daemon back up:
to stop:              ${GREEN}xchainz-cli stop${NC}
to start:             ${GREEN}xchainzd${NC}
to edit:              ${GREEN}nano ~/.xchainz/xchainz.conf${NC}
to check mn status:   ${GREEN}xchainz-cli getmasternodestatus${NC}
========================================================================
